...
export default function ContactSection() {
  ...
  useGSAP(
    () => {
      ...
      const moveGlowX = gsap.quickTo(glow, "x", { duration: 0.5, ease: "power3.out" });
      const moveGlowY = gsap.quickTo(glow, "y", { duration: 0.5, ease: "power3.out" });
      ...
      const handleMouseLeave = () => { rotateX(0); rotateY(0); };

      section.addEventListener("mousemove", handleMouseMove);
      section.addEventListener("mouseleave", handleMouseLeave);

      gsap.from(".contact-letter", {
        y: 80, scaleY: 0.4, scaleX: 0.75, opacity: 0, fontWeight: 300,
        duration: 0.8, stagger: 0.025, ease: "power4.out", transformOrigin: "bottom center",
      });
      gsap.from(".contact-card", { y: 80, opacity: 0, duration: 1, delay: 0.3, ease: "power3.out" });
      gsap.from(".contact-field", { y: 25, opacity: 0, duration: 0.7, stagger: 0.1, delay: 0.6, ease: "power3.out" });
      ...
      letters.forEach((letter, hoveredIndex) => {
        letter.addEventListener("mouseenter", () => {
          letters.forEach((target, index) => {
            ...
            let fontWeight = 300, scaleY = 0.72, opacity = 0.35, y = 0;
            if (distance === 0)      { fontWeight = 900; scaleY = 1.18; opacity = 1;    y = -5; }
            else if (distance === 1) { fontWeight = 700; scaleY = 1.08; opacity = 0.85; y = -2; }
            else if (distance === 2) { fontWeight = 500; scaleY = 0.98; opacity = 0.65; }
            else if (distance === 3) { fontWeight = 400; scaleY = 0.88; opacity = 0.5;  }
            gsap.to(target, { fontWeight, scaleY, opacity, y, duration: 0.25, ease: "power2.out", transformOrigin: "bottom center" });
          });
        });
      });
      ...
      const resetLetters = () => {
        gsap.to(".contact-letter", {
          fontWeight: 300, scaleY: 1, opacity: 1, y: 0,
          duration: 0.35, ease: "power2.out", stagger: 0.01, transformOrigin: "bottom center",
        });
      };
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch("https://formspree.io/f/mgoblglv", {
        method: "POST",
  ...
  return (
    ...
    >
      ...
      {status !== "idle" && status !== "loading" && (
        <div className={`fixed bottom-8 left-1/2 z-50 -translate-x-1/2 rounded-2xl border px-6 py-4 text-sm font-medium shadow-xl backdrop-blur-xl transition-all ${
          status === "success"
      ...
      <div className="relative z-10 mx-auto grid min-h-[calc(100vh-12rem)] max-w-7xl items-center gap-14 lg:grid-cols-[1fr_520px]">
        <div>
          <p className="mb-6 text-sm uppercase tracking-[0.4em] text-[#4DA3FF]">Contact</p>
        ...
        >
          ...
          <div className="contact-field mb-5">
            ...
            <input
              ...
              className="w-full rounded-2xl border border-white/10 bg-black/40 px-5 py-4 text-white outline-none focus:border-[#4DA3FF] invalid:border-red-500/50"
            />
          </div>

          <div className="contact-field mb-5">
            <label className="mb-2 block text-sm text-white/50">Project Type</label>
            <select
              name="project_type"
              className="w-full rounded-2xl border border-white/10 bg-black/40 px-5 py-4 text-white outline-none focus:border-[#4DA3FF]"
            >
          ...
          <button
            ...
            className="contact-field group relative w-full overflow-hidden rounded-full bg-[#4DA3FF] px-8 py-4 font-semibold text-black disabled:opacity-60"
          >
